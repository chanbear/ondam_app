...
/// Guardian UI Application Round 1 §4 — Guardian도 Senior(`LoginNotifier`)와
/// 동일하게 휴대폰 번호 + 비밀번호(PIN)를 하나의 로그인 화면에서 입력받아,
/// 기존 인증 인프라(signUp/hasPin/setPin/verifyPin/addRole usecase와 그 뒤의
/// Edge Function)를 그대로 순서대로 호출하는 오케스트레이션 계층. 화면
/// 전환 없이 이 하나의 제출로 가입/로그인/PIN 최초 설정/PIN 확인/역할
/// 자동 결정까지 모두 끝난다.
///
...
class LoginNotifier extends AsyncNotifier<void> {
  ...
  Future<Result<PinVerifyResult>> _submit({
    ...
    // Guardian 로그인 화면에도 이름 입력이 없다(Senior와 동일한 결정). 서버는
    // signup-with-phone 호출 시 이름을 필수로 요구하지만, 이 값은
    // auth.users의 메타데이터로만 저장되고 앱 어디에서도 읽거나 표시하지
    // 않는다 — DB 스키마를 바꾸지 않고 화면에도 노출하지 않기 위해 고정값을
    // 사용한다.
    final signUpResult = await ref
        .read(signUpUseCaseProvider)
        .call(name: '온담 보호자', rawPhoneNumber: rawPhoneNumber);
    ...
    // Guardian 앱은 역할을 물어보지 않고 보호자 역할을 자동으로 부여한다
    // (Senior가 어르신 역할을 자동 부여하는 것과 동일한 패턴). 이미
    // 역할이 있으면(재로그인) 다시 추가하지 않는다 — user_roles의
    // (user_id, role) unique 제약을 그대로 존중한다.
