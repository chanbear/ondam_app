...
/// 접근성 설정(글자크기/음성안내) — 순수 로컬 기기 설정이라 `LocalStorageService`에
/// 저장한다(서버 `users` 테이블이 아직 없음 — profile 관련 값과 다른 이유로
/// "지금은 로컬에만 둔다"가 아니라, 이 값들은 애초에 기기별 설정이라 서버에
/// 둘 필요가 없다). `easyModeProvider`와 동일한 패턴(core/easy_mode 참고).
