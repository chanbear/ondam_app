...
>
> - **v18(이 버전)**: 실기기(Galaxy S22) 테스트로 Supabase Phone OTP가 SMS 발송업체 미연동으로 항상 실패함을 발견하고(`Unsupported phone provider`), 사용자 결정에 따라 **OTP를 완전히 제거**하고 "이름+전화번호 즉시 가입/로그인"으로 Phase 2 인증 방식을 변경했다 — 전화번호 소유 검증이 없어지는 트레이드오프를 사용자가 인지하고 확정한 영구 결정이다. 상세 메커니즘·이유는 `technical-decisions.md` §1-3-B(v18). Senior/Guardian 양쪽 `features/auth`의 OTP 코드를 제거하고 `SignUpUseCase`/`SignUpNotifier`로 교체했으며, `PhoneInputPage`에 이름 입력을 추가하고 `PinForgotPage`는 OTP 재인증 대신 동일 메커니즘의 무입력 재로그인으로 바뀌었다. 신규 Edge Function `signup-with-phone`(서버 시크릿 `PHONE_SIGNUP_SECRET` 필요)은 **아직 배포/시크릿 설정 전** — 코드만 작성했다. 상세는 아래 Phase 2 항목(v18 추가 보고).
>
> - **v20(이 버전)**: v19 이후 문서화되지 않은 채 진행된 작업을 이번에 기록한다. **(1) 일정(schedule) 기능 구현 완료** — `schedules` 테이블(본인 전체 CRUD + accepted guardian read-only, migration `20260824000001`)을 신규 작성하고, Senior "기록 탭"과 Guardian "다가오는 일정" 양쪽을 실제 데이터로 연결했다(§1 feature 구조 표에서 계획만 돼 있던 `schedule`이 이제 실제 구현임). **(2) `welfare_center`(복지센터 찾기)는 §5 Phase 4/10에 "미구현"으로 기록돼 있었지만 실제로는 domain/data/presentation 전 계층 + `search-welfare-centers` Edge Function까지 이미 구현·배포돼 있음을 코드로 재확인했다** — 그 기록은 stale였다. **(3) Phase 42~43에서 승인된 프로토타입 디자인(ONDAM 2.0)을 Guardian/Senior 화면 전반에 실제로 적용했다** — `design_system` 토큰 확장(`AppTouch`/`AppAnalysisComponents`), 양쪽 앱 l10n 확장, Guardian 로그인 화면을 Senior와 동일하게 전화번호+PIN 입력/최초 설정/재확인을 화면 하나로 통합(`LoginNotifier`), 일부 홈 탭의 `IndexedStack` 0x0 constraint 레이아웃 버그 수정. **(4) Support 화면에 관할 행정복지센터 연락처 조회 기능을 신규 추가**(`search-local-government-contact`, 지역 미등록 시 등록 유도, 전화번호 없는 데이터는 정직하게 비워둠). **(5) 가장 중요한 정정**: v7~v19에 반복 기록된 "Supabase 프로젝트가 없어 NOT AVAILABLE"은 이 시점 기준 stale하다 — 실제로는 2026-08-13경부터 실제 프로젝트가 연결돼 있었다(상세는 `technical-decisions.md` v20). 이번 세션에 배포 누락 상태였던 마이그레이션 4개·Edge Function 3개를 원격에 반영해, 현재 마이그레이션 20/20·Edge Function 15/15가 로컬 코드와 완전히 동기화됐다. 위 (1)(3)(4) 작업 각각의 `dart format`/`flutter analyze`/`flutter test`는 이번 세션에 재검증 완료(Senior 392개, Guardian 143개, 양쪽 0 issue). 실기기(Push 수신+딥링크, 문서/문자 AI 분석) E2E 검증은 여전히 이번 범위 밖 — 별도 세션에서 진행.
>
...
>
> **v18 추가 보고 (OTP 제거)**: v7이 "검증하지 못했다"고 남겨뒀던 실제 서버 통신을 라이브 프로젝트+실기기로 처음 확인해보니, `signInWithOtp`가 SMS 발송업체 미연동으로 항상 실패했다(`Unsupported phone provider`). Twilio 등 SMS 발송업체는 실사업자 계정·본인인증·결제수단이 필요해 이 세션에서 즉시 해결할 수 없어, **사용자가 OTP 자체를 제거하는 것으로 결정**했다(전화번호 소유 미검증이라는 보안 트레이드오프를 인지하고 확정 — PIN/idle timeout은 그대로라 가입 이후 보호 수준은 동일). 신규 Edge Function `signup-with-phone`이 서버 전용 시크릿(`PHONE_SIGNUP_SECRET`)으로 사용자별 결정론적 비밀번호를 만들어 phone+password 그랜트로 세션을 발급하고, 클라이언트는 `refresh_token`만 받아 `setSession()`한다 — 메커니즘 상세는 `technical-decisions.md` §1-3-B. `PhoneInputPage`(이름 필드 추가, 이제 가입/로그인 겸용 단일 화면), `PinForgotPage`(OTP 재인증 단계를 삭제하고 화면 진입 즉시 무입력 재로그인으로 대체)를 포함해 Senior/Guardian 양쪽 `features/auth`를 함께 수정했다. `dart format`/`flutter analyze`/`flutter test` 통과 확인은 이 보고 직후 절 참고. **아직 안 된 것**: `signup-with-phone` Edge Function 배포(`supabase functions deploy`)와 `PHONE_SIGNUP_SECRET` 시크릿 설정 — 코드만 작성했고 라이브 배포는 사용자 확인 후 별도 진행한다.
