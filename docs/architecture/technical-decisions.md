...
>
> - **v20(이 버전)**: v19까지 반복적으로 기록된 "Supabase 프로젝트가 없어 NOT AVAILABLE"은 이 시점 기준 **stale하다** — 실제로는 2026-08-13경부터 실제 Supabase 프로젝트(`chanbear's Project`, ref `kmexnrcelsxnxplftqan`, ap-northeast-2, `ACTIVE_HEALTHY`)가 연결되어 있었고, `ANTHROPIC_API_KEY`/`DATA_GO_KR_SERVICE_KEY`/`FIREBASE_SERVICE_ACCOUNT_JSON`/`PHONE_SIGNUP_SECRET` 등 시크릿도 이미 설정돼 있었다. 다만 최근 작업 3건(v19 정보 탭 혜택 검색, 일정 기능, 지역 행정복지센터 연락처)이 마이그레이션/Edge Function 배포에서 누락돼 있어, 이번 세션에 마이그레이션 4개(`20260821000001_guardian_contact_phone`/`20260822000001_users_name_age`/`20260823000001_users_gender`/`20260824000001_create_schedules`)와 Edge Function 3개(`search-benefit-services`/`get-benefit-service-detail`/`search-local-government-contact`)를 원격에 배포했다 — 현재 마이그레이션 20/20, Edge Function 15/15가 로컬 코드와 완전히 동기화된 상태다(`supabase migration list`/`supabase functions list`로 확인). §4의 `schedules` 테이블도 "아직 합의 수준의 개요"가 아니라 위 migration으로 실제 구현·배포 완료됐다. **앞으로 이 문서에 Supabase 연결 상태를 "NOT AVAILABLE"로 적기 전에는 반드시 `supabase migration list`/`supabase functions list`로 실제 원격 상태를 먼저 확인할 것** — 문서만 보고 판단하지 않는다. 실기기(Push 수신+딥링크, 문서/문자 AI 분석) E2E 검증 자체는 여전히 진행 중이며 이번 버전의 범위가 아니다. `implementation-plan.md`도 Phase 11(2026-08-14) 이후 실제 완료된 작업(정보 탭/일정/복지센터 찾기/UI 리디자인 Round 1/지역센터 연락처)이 반영되지 않은 채 멈춰 있었다는 점을 함께 기록한다 — 별도로 갱신 필요.
...
| 테이블(가칭) | 핵심 컬럼(예시) | 비고 |
|---|---|---|
| `users` | id, phone, name, age, region, easy_mode_enabled | **v5: `pin_hash`/`role` 컬럼 제거** — 각각 `pin_credentials`, `user_roles`로 분리(§1-3-A). Phase 2에는 `auth.users`(Supabase Auth 내장)만 있고 이 앱 소유의 `users` 테이블은 아직 만들지 않았다 — 프로필 필드(name/age/region 등)가 필요해지는 Phase에서 생성. `name`/`age` 컬럼은 `20260822000001`에서, `gender` 컬럼은 v19에서 실제로 생성됨 — easy_mode_enabled는 여전히 미생성 |
...
25. **(v19 신규) 지자체복지서비스/중앙부처복지서비스 API의 실제 요청/응답 필드명과 성별 필터 지원 여부** — SWAGGER 문서를 로그인 없이 확인하지 못해 `benefit_service_client.ts`/`benefit_service_detail_client.ts`의 필드명이 미확인 초안이다. 서비스키 발급 후 라이브 검증 필요(`welfare_center`의 PHASE 26→35와 동일한 절차).
